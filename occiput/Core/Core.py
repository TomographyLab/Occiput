...
__all__ = ['make_grid','transform_grid','grid_from_box_and_affine',
           'resample_image_on_grid', 'Transform_Affine', 'Transform_6DOF',
           'Transform_Identity','Transform_Rotation','Transform_Scale',
           'Transform_Translation','GridND','Grid3D','Image3D','ImageND']
...
from ..Visualization import MultipleVolumes,MultipleVolumesNiftyPy, \
    VolumeRenderer,ipy_table,has_ipy_table
from . import transformations as tr
from ..global_settings import printoptions,get_default_background, \
    is_gpu_enabled
from ..Functional.NiftyRec import INTERPOLATION_LINEAR,INTERPOLATION_POINT, \
...
class Transform_Affine(object):
    ...
    def left_multiply(self,obj):
        ...
        if not self.can_left_multiply(obj):
            ...
            print(
                    (
                            "Affine is incompatible with the given object: composing [%s,%s] \
                        with [%s,%s] is not allowed. "
                            % (
                                self.map_to,self.map_from,
                                self.__space_of_obj(obj),
                                X)
                    )
                    )
    ...
    def from_translation_rotation(self,tra,rot,axis = [0,0,1]):
        return numpy.dot(tr.translation_matrix(tra),
...
class Transform_Scale(
        Transform_Affine
        ):  # FIXME: figure out what transformations.py means by scale matrix
...
class GridND(object):
    ...
    def mean_distance_from_point(self,point,use_corners_only = True):
        if use_corners_only:
            corners = self.corners()
            dist = corners - numpy.tile(
                    numpy.asarray(point).reshape(3,1),[1,corners.shape[1]]
                    )
    ...
    def corners(self,homogeneous_coords = False):
        if self.__corners is None:
            ...
            for i in range(n_corners):
                ...
                b = (numpy.asarray(
                        self.data.shape[0: self.ndim]) - 1) * numpy.asarray(
                        b
                        )
                s = str(b.tolist())[1:-1]
...
class ImageND(object):
    ...
    def get_world_grid(self,n_points = None):
        ...
        grid = grid_from_box_and_affine(
                self.get_world_grid_min(),self.get_world_grid_max(),n_points
                )
        return grid
    ...
    def get_pixel_grid(self):
        ...
        grid = grid_from_box_and_affine(numpy.asarray([0,0,0]),n_points - 1,
                                        n_points)
        return grid
...
class Image3D(ImageND):
    ...
    def compute_resample_on_grid(
        ...
        resampled_data = resample_image_on_grid(
                self,
                grid,
        ...
        return Image3D(
                data = resampled_data
                )  # FIXME: perhaps just return the raw resampled data
    ...
    def compute_gradient_on_grid(
            self,grid,affine_grid_to_world = None,verify_mapping = True
            ):
        resampled_data = resample_image_on_grid(
                self,
                grid,
    ...
    def export_image(
            self,index,axis = 0,normalise = True,scale_factor = None,
            shrink = None,rotate = 0
            ):
        pass
    ...
    def display(
        ...
        D = MultipleVolumes(
                ...
                shrink = shrink,
                rotate = rotate,
                subsample_slices = subsample_slices,
                scales = scales,
    ...
    def __add__(self,other):
        if self._is_same_type(other):
            if self._is_in_same_space(other):
                ...
                # FIXME: raise error
                print(
                        "SUM of images not in the same space. It cannot be done. ")
        else:
            out = self.copy()
    ...
    def __sub__(self,other):
        if self._is_same_type(other):
            if self._is_in_same_space(other):
                ...
                # FIXME: raise error
                print(
                        "SUB of images not in the same space. It cannot be done. ")
        else:
            out = self.copy()
            out.data = out.data - other
            return out
    ...
    def __mul__(self,other):
        if self._is_same_type(other):
            if self._is_in_same_space(other):
                ...
                # FIXME: raise error
                print(
                        "MUL of images not in the same space. It cannot be done. ")
        else:
            out = self.copy()
            out.data = out.data * other
            return out
    ...
    def __div__(self,other):
        if self._is_same_type(other):
            if self._is_in_same_space(other):
                ...
                # FIXME: raise error
                print(
                        "DIV of images not in the same space. It cannot be done. ")
        else:
            out = self.copy()
            out.data = out.data / other
            return out
...
def grid_from_box_and_affine(
        min_coords,max_coords,n_points,affine = None,space = "world"
        ):
