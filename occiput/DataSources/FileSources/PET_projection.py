...
def import_interfile_projection_data(
    ...
    if load_time:
        try:
            ...
            print(
                "Unable to load image (sinogram) duration. This may determine an incorrect scale and use of randoms and scatter when reconstructing. Set .time_bins manually. "
            )
            duration = int32([0, 0])
    else:
        duration = int32([0, 0])
...
def export_interfile_projection(
    ...
    N_sinograms = (
        int32(michelogram.segments_sizes[0])
        + 2 * int32(michelogram.segments_sizes[1::]).sum()
    )
    ...
    for i in range(N_azim):
        index1 = index0 + z_elements[i]
        data_azim = zeros(
            [index1 - index0, data.shape[1], data.shape[2]], dtype=float32, order="C"
        )
        for j in range(N_axial):
