...
def PET_project(activity,attenuation,binning,
    ...
    descriptor = [{'name':'activity','type':'array','value':activity},
                  {
                      'name':'activity_size_x','type':'int',
                      'value':activity.shape[0]
                  },
                  {
                      'name':'activity_size_y','type':'int',
                      'value':activity.shape[1]
                  },
                  {
                      'name':'activity_size_z','type':'int',
                      'value':activity.shape[2]
                  },

                  {'name':'attenuation','type':'array','value':attenuation},
                  {
                      'name':'attenuation_size_x','type':'int',
                      'value':attenuation.shape[0]
                  },
                  {
                      'name':'attenuation_size_y','type':'int',
                      'value':attenuation.shape[1]
                  },
                  {
                      'name':'attenuation_size_z','type':'int',
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'PET_project' was unsuccessful.",r.status,
            'niftyrec_c.PET_project')
...
def PET_backproject(projection_data,attenuation,binning,use_gpu = 0):
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'PET_backproject' was unsuccessful.",r.status,
...
def PET_project_compressed(activity,attenuation,offsets,locations,active,
                           ...
                           attenuation_size_z,
                           T_activity_x,T_activity_y,T_activity_z,R_activity_x,
                           R_activity_y,R_activity_z,
    ...
    descriptor = [{
                  ...
                  {
                      'name':'activity_size_x','type':'float',
                      'value':activity_size_x
                  },
                  {
                      'name':'activity_size_y','type':'float',
                      'value':activity_size_y
                  },
                  {
                      'name':'activity_size_z','type':'float',
                      'value':activity_size_z
                  },
                  ...
                  {
                      'name':'attenuation_size_x','type':'float',
                      'value':attenuation_size_x
                  },
                  {
                      'name':'attenuation_size_y','type':'float',
                      'value':attenuation_size_y
                  },
                  {
                      'name':'attenuation_size_z','type':'float',
                      'value':attenuation_size_z
                  },

                  {
                      'name':'T_attenuation_x','type':'float',
                      'value':T_attenuation_x
                  },
                  {
                      'name':'T_attenuation_y','type':'float',
                      'value':T_attenuation_y
                  },
                  {
                      'name':'T_attenuation_z','type':'float',
                      'value':T_attenuation_z
                  },
                  {
                      'name':'R_attenuation_x','type':'float',
                      'value':R_attenuation_x
                  },
                  {
                      'name':'R_attenuation_y','type':'float',
                      'value':R_attenuation_y
                  },
                  {
                      'name':'R_attenuation_z','type':'float',
                      'value':R_attenuation_z
                  },
                  ...
                  {
                      'name':'truncate_negative_values','type':'uint',
                  ...
                  {
                      'name':'time_profiling','type':'array','value':None,
                      'dtype':np.float32,'size':(7)
                  },
                  ]
...
def PET_backproject_compressed(projection_data,attenuation,offsets,locations,
    ...
    if attenuation is None:
        attenuation = np.zeros((0,0,0))
    descriptor = [{
                      'name':'back_projection','type':'array','value':None,
                      'dtype':np.float32,
                      'size':(N_activity_x,N_activity_y,N_activity_z),
                      'order':"F"
                  },
                  ...
                  {'name':'active','type':'array','value':active},
                  {
                      'name':'projection_data','type':'array',
                      'value':projection_data
                  },
                  ...
                  {'name':'sample_step','type':'float','value':sample_step},
                  {
                      'name':'background_activity','type':'float',
                      'value':background
                  },
                  {
                      'name':'background_attenuation','type':'float',
    ...
    descriptor = [
        {
            ...
            'size':(voxels[0],voxels[1],voxels[2]),
            'order':"F"
        },
...
def ET_cylindrical_phantom(voxels,size,center,radius,length,axis,inner_value,
                           outer_value):
...
def ET_spheres_ring_phantom(voxels,size,center,ring_radius,min_sphere_radius,
...
def SPECT_backproject_parallelholes(projection,cameras,attenuation = None,
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'SPECT_backproject_parallelholes' was unsuccessful.",
            r.status,
            'niftyrec_c.SPECT_backproject_parallelholes')
    return r.dictionary['backprojection']


def CT_project_conebeam(attenuation,camera_trajectory,source_trajectory,
                        use_gpu = 0):
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'CT_project_conebeam' was unsuccessful.",
            r.status,
            'niftyrec_c.CT_project_conebeam')
    return r.dictionary


def CT_backproject_conebeam(projection_data,camera_trajectory,
                            source_trajectory,use_gpu = 0):
    """Transmission imaging back-projection; cone-beam geometry. """
    descriptor = []
    r = call_c_function(niftyrec_c.CT_backproject_conebeam,descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'CT_backproject_conebeam' was unsuccessful.",
            r.status,
            'niftyrec_c.CT_backproject_conebeam')
    return r.dictionary


def CT_project_parallelbeam(attenuation,camera_trajectory,source_trajectory,
                            use_gpu = 0):
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'CT_project_parallelbeam' was unsuccessful.",
            r.status,
            'niftyrec_c.CT_project_parallelbeam')
    return r.dictionary


def CT_backproject_parallelbeam(attenuation,camera_trajectory,
                                source_trajectory,use_gpu = 0):
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'CT_backproject_parallelbeam' was unsuccessful.",
            r.status,
...
def TR_grid_from_box_and_affine(box_min,box_max,box_n,affine_box2grid = None):
    ...
    if affine_box2grid is None:
        affine_box2grid = np.eye(4,dtype = np.float32)
    descriptor = [
        {
            'name':'grid','type':'array','value':None,'dtype':np.float32,
            'size':(box_n[0],box_n[1],box_n[2],3),
            'order':"F"
        },
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'TR_grid_from_box_and_affine' was unsuccessful.",
            r.status,
            'niftyrec_c.TR_grid_from_box_and_affine')
    return r.dictionary['grid']


def TR_resample_grid(image_array,grid_array,affine_index2grid = None,
                     background = 0.0,use_gpu = 1,
    ...
    if affine_index2grid is None:
        affine_index2grid = np.eye(4,dtype = np.float32)
    resampled_shape = np.asarray(
            [grid_array.shape[0],grid_array.shape[1],grid_array.shape[2]])
    descriptor = [{
                      'name':'resampled_array','type':'array','value':None,
                      'dtype':np.float32,
                      'size':(resampled_shape[0],resampled_shape[1],
                              resampled_shape[2]),'order':"F"
                  },
                  {
                      'name':'image_array','type':'array','value':image_array,
                      'dtype':np.float32
                  },
                  {
                      'name':'affine','type':'array','value':affine_index2grid,
                      'dtype':np.float32
                  },
                  {
                      'name':'grid_array','type':'array','value':grid_array,
                      'dtype':np.float32
                  },
                  ...
                  {
                      'name':'interpolation_mode','type':'uint',
                      'value':np.uint32(interpolation_mode)
                  },
                  ]
    r = call_c_function(niftyrec_c.TR_resample_grid,descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'TR_resample_grid' was unsuccessful.",r.status,
            'niftyrec_c.TR_resample_grid')
    return r.dictionary['resampled_array']


def TR_resample_box(image_array,box_min,box_max,box_n,affine_index2grid = None,
                    background = 0,use_gpu = 1,
                    interpolation_mode = INTERPOLATION_LINEAR):
    pass


def TR_gradient_grid(image_array,grid_array,affine_index2grid = None,
                     background = 0,use_gpu = 1,
                     interpolation_mode = INTERPOLATION_LINEAR):
    pass


def TR_gradient_box(image_array,box_min,box_max,box_n,affine_index2grid = None,
                    background = 0,use_gpu = 1,
                    interpolation_mode = INTERPOLATION_LINEAR):
    pass
...
def TR_transform_grid(grid_array,affine_from_grid,use_gpu = 1):
    ...
    if affine_from_grid is None:
        affine_from_grid = np.eye(4,dtype = np.float32)
    descriptor = [{
                      'name':'transformed_array','type':'array','value':None,
                      'dtype':np.float32,
                      'size':(grid_array.shape[0],grid_array.shape[1],
                              grid_array.shape[2],3),'order':"F"
                  },
                  {
                      'name':'grid_array','type':'array','value':grid_array,
                      'dtype':np.float32
    ...
    if not r.status == status_success():
        raise ErrorInCFunction(
            "The execution of 'TR_transform_grid' was unsuccessful.",r.status,
