...
if found == NOT_FOUND:
    ...
    print("1) Before launching Python, type the following in the terminal ("
    "the same terminal): ")
...
def resample_image_rigid(image_data, translation, rotation, center_rotation, sform=None, use_gpu=1):
    ...
    if sform is None:
        sform = np.eye(4, dtype=np.float32)
    descriptor = [{'name': 'image_data', 'type': 'array', 'value': image_data, 'dtype': np.float32},
                  {'name': 'resampled_image_data', 'type': 'array', 'value': None, 'dtype': np.float32,
                  ...
                  {'name': 'translation', 'type': 'array', 'value': translation, 'dtype': np.float32},
                  {'name': 'rotation', 'type': 'array', 'value': rotation, 'dtype': np.float32},
                  {'name': 'center_rotation', 'type': 'array', 'value': center_rotation, 'dtype': np.float32},
                  {'name': 'sform', 'type': 'array', 'value': sform, 'dtype': np.float32},
                  {'name': 'use_gpu', 'type': 'int', 'value': np.uint32(use_gpu)}, ]
...
def deriv_intensity_wrt_space_rigid(image_data, translation, rotation, center_rotation, sform=None, use_gpu=1):
    ...
    if sform is None:
        sform = np.eye(4, dtype=np.float32)
    descriptor = [{'name': 'image_data', 'type': 'array', 'value': image_data, 'dtype': np.float32},
                  {'name': 'gradient', 'type': 'array', 'value': None, 'dtype': np.float32,
