...
class RigidTransformationSSD:
    ...
    def _contruct_ilang_model(self):
        ...
        self.graph.add_dependence(
            self.ilang_model,
            {"target": "target", "source": "source", "transformation": "transformation"},
        )
    ...
    def estimate_transformation(
        ...
        last_sample = self.sampler.sample_node(
            "transformation", nsamples=iterations, trace=trace
        )
    ...
    def display_in_browser(
        self, axis=0, shrink=256, rotate=90, subsample_slices=4, scale_factors=None
    ):
